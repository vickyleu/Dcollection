...
@Suppress("UNCHECKED_CAST")
fun ListView.swapAdapter(): Consumer<ListAdapter> {
    return Consumer { it ->
...
fun ListView.paging(path: String): PropertyBinding = commandBinding(path, paging(), Consumer { (this.adapter as? AdapterPagingCompleteListener)?.onPagingComplete(!it) })
fun ListView.itemClick(path: String): PropertyBinding = commandBinding(path, RxAdapterView.itemClicks(this), Consumer {})

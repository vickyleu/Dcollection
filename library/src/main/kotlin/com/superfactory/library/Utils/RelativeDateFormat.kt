...
fun ms2RelativeDate(context: Context, time: Long): String {
    ...
    if (delta < ONE_MINUTE) {
        ...
        return context.getString(R.string.one_second_ago, (if (seconds <= 0) 1 else seconds).toString())
    }
    if (delta < 45 * ONE_MINUTE) {
        val minutes = toMinutes(delta)
        return context.getString(R.string.one_minute_ago, (if (minutes <= 0) 1 else minutes).toString())
    }
    if (delta < ONE_DAY) {
        val hours = toHours(delta)
        return context.getString(R.string.one_hour_ago, (if (hours <= 0) 1 else hours).toString())
    }
    if (delta < 30 * ONE_DAY) {
        val days = toDays(delta)
        return context.getString(R.string.one_day_ago, (if (days <= 0) 1 else days).toString())
    }
    if (delta < ONE_YEAR) {
        val months = toMonths(delta)
        return context.getString(R.string.one_month_ago, (if (months <= 0) 1 else months).toString())
    } else {
        val years = toYears(delta)
        return context.getString(R.string.one_year_ago, (if (years <= 0) 1 else years).toString())
    }
}

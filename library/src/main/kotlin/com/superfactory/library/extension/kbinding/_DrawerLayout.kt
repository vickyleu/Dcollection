...
fun DrawerLayout.drawerOpen(gravity: Int, vararg paths: String, mode: OneWay = OneWay(), converter: OneWayConverter<*, Boolean> = EmptyOneWayConverter1()) = oneWayPropertyBinding(paths, RxDrawerLayout.open(this, gravity), false, converter)

fun DrawerLayout.drawerOpen(gravity: Int, path: String, mode: TwoWay, converter: TwoWayConverter<Boolean, Any?> = EmptyTwoWayConverter()) = twoWayPropertyBinding(path, RxDrawerLayout.drawerOpen(this, gravity), RxDrawerLayout.open(this, gravity), converter)